...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        int previousVert = iter.next();
        int currentVert = iter.next();
        Road previousRoad = _map.getLabel(previousVert, currentVert);

        String previousDir = previousRoad.direction().fullName();
        String currentDir = "";

        String previousName = previousRoad.toString();
        String currentName = "";

        double previousDist = previousRoad.length();
        ...
        while (iter.hasNext()) {
            ...
            Road currentRoad = _map.getLabel(previousVert, currentVert);
            currentDir = currentRoad.direction().fullName();
            currentName = currentRoad.toString();
            currentDist = currentRoad.length();

            if (!currentDir.equals(previousDir)
                    && !currentName.equals(previousName)) {
                System.out.printf("%d. Take %s %s for %.1f miles.\n",
                        seq, previousName, previousDir, previousDist);
            ...
            Location afinal = _map.getLabel(currentVert);
            finalName = afinal.toString();
        }

        System.out.printf("%d. Take %s %s for %.1f miles to %s.\n",
                seq, currentName, currentDir, previousDist, finalName);
        seq += 1;
        return seq;
    }
    ...
    private void addRoad(String from, String name, double length,
        ...
        Road backward = new Road(name, dir.reverse(), length);
        _map.add(v0, v1, forward);
        _map.add(v1, v0, backward);
    }
