...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command("manual\\s+(\\S+)$", this::doManual),
        new Command("new$", this::doNew),
        new Command("(\\S+)\\s+(\\S+)\\s+(\\S+)$", this::doMove),
        new Command("(\\S+)\\W+(\\S+)\\W+(\\S+)\\W+$", this::doMove),
        new Command("undo$", this::doUndo),
        new Command("auto\\s+(\\S+)$", this::doAuto)
    };
    ...
    /** Make the C player a human player.
     * @param mat mat
     * */
    private  void doManual(Matcher mat) {
        if (mat.group(1).toUpperCase().equals("WHITE")) {
            _white = _manualPlayerTemplate.create(WHITE, this);
        } else if (mat.group(1).toUpperCase().equals("BLACK")) {
    ...
    private void doMove(Matcher mat) {
        ...
        Square fromSQ = Square.sq(from);
        Square toSQ = Square.sq(to);
        Square spearSQ = Square.sq(spear);

        if (!board().isUnblockedMove(fromSQ, toSQ, spearSQ)) {
            return;
        } else if (!board().get(fromSQ).equals(board().turn())) {
            return;
        } else if (!board().isUnblockedMove(toSQ, spearSQ, fromSQ)) {
            return;
        }

        board().makeMove(fromSQ, toSQ, spearSQ);
    ...
    /** Undo a single move on the board. */
