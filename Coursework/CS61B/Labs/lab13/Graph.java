...
public class Graph {
    ...
    public int[] dijkstras(int startVertex) {
        ...
        PriorityQueue<Integer> f = new PriorityQueue<>(vertexCount, Comparator.comparing(_v -> d[_v]));
        ...
        while (!f.isEmpty()) {
            ...
            if (!v.contains(vert)) {
                for (Edge e : adjLists[vert]) {
                    if (e.edgeWeight + d[vert] < d[e.to()]) {
                        d[e.to()] = e.edgeWeight + d[vert];
                    }
                    f.add(e.to());
                }
            }
