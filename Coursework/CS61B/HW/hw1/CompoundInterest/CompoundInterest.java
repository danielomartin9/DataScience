...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
        ...
        int yearDifference = numYears(targetYear); // Difference between targetYear and THIS_YEAR
        presentValue = presentValue * Math.pow((1 + rate / 100), yearDifference);
        return presentValue;
    }
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        double nominalValue = futureValue(presentValue, rate, targetYear); // Nominal value calculated by futureValue
        int yearDifference = numYears(targetYear); //Difference between targetYear and THIS_YEAR

        nominalValue = nominalValue * Math.pow((1 - inflationRate / 100), yearDifference);
        return nominalValue;
    }

    /** Scuppose you invest PERYEAR dollars at the end of every year until
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        int yearDifference = numYears(targetYear); //Difference between targetYear and THIS_YEAR
        double totalSavings = 0; //Initialize container for result

        for(int j = yearDifference; j >= 0; j--){
           totalSavings = totalSavings + (perYear * Math.pow((1 + rate / 100), j));
        }
        return totalSavings;
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        double totalSavings = totalSavings(perYear, targetYear, rate); // Result returns totalSavings by calling method totalSavings
        int yearDifference = numYears(targetYear); // Difference between targetYear and THIS_YEAR

        double totalSavingsReal = totalSavings * Math.pow((1 - inflationRate / 100), yearDifference); // totalSavings converted to current year dollars
